...
export const transferMnt = async ({
  toAddress,
  amount
}: {
  toAddress: string;
  ...
  try {
    ...
    if (currentBalance < totalRequired) {
      ...
      throw new Error(
        `Insufficient MNT balance. Current: ${currentBalanceFormatted} MNT, ` +
        `Required: ${totalRequiredFormatted} MNT (${amount} MNT + ${gasCostFormatted} MNT gas)`
      );
    }

    console.log(`Transferring ${amount} MNT to ${toAddress}...`);
    ...
    console.log(`MNT transfer transaction sent: ${tx.hash}`);
    ...
    console.log(`✅ MNT transfer completed successfully. Transaction hash: ${tx.hash}`);
    return tx.hash;
    
  } catch (error: any) {
    console.error("MNT transfer failed:", error.message);
    ...
    if (error.code === 'INSUFFICIENT_FUNDS') {
      ...
      throw new Error("Insufficient MNT balance for transfer and gas fees");
    } else if (error.message.includes("gas")) {
      throw new Error("Gas estimation failed or gas limit exceeded");
    }
    
    throw new Error(`MNT transfer failed: ${error.message}`);
  }
};
