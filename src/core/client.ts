...
if (!process.env.MANTLE_RPC_URL || !process.env.PRIVATE_KEY) {
  throw new Error("Missing MANTLE_RPC_URL or PRIVATE_KEY in .env file");
}

const provider: Provider = new ethers.JsonRpcProvider(process.env.MANTLE_RPC_URL);
